...
let write_message flow msg =
  ...
  @@ fun writer -> Eio.Buf_write.string writer (msg ^ "\n")
;;
...
let handle_session room flow _ =
  ...
  let username =
    ...
    | _ -> ""
  in
  if not (Str.string_match valid_username_re username 0)
  then ()
  else (
    ...
    let rec loop () =
      try
        ...
        loop ()
      with
      | End_of_file | Failure _ -> Room.remove_user room username
    in
    loop ())
;;
